...
Focus follows mouse (2026-09-25): all 48 core tests passed, including preference default/persistence, content hit testing, hidden targets, tab/divider exclusions, stationary-pointer keyboard focus and suppression policy. Strict all-target Clippy and the application build passed. Full-app pointer routing, live setting changes, inactive windows and modal/drag interactions still need manual UI verification; the installed GPUI version does not expose a usable external event-dispatch return type for the attempted diagnostic harness. No polling or synthetic clicks are used.

Single application host (2026-09-25): all 52 core Rust/Neovim tests passed; strict all-target Clippy and the application build passed. Two runs of the real macOS launcher test verified concurrent startup shares one GUI PID, distinct Neovim PIDs, caller cwd/environment and spaced filenames, independent per-window `--wait`, and (second run) direct/detached handoff. No user windows were closed by the test. Linux desktop runtime validation remains outstanding. See `docs/INSTANCES.md`.


## Resize coalescing, terminal search and Window menu, 2026-09-26

`cargo run --locked --example resize_benchmark` compares notification-per-size with one in-flight resize and a latest-only pending size, using real bundled Neovim. It sends 100 sizes at 2 ms intervals, with and without a simulated 25 ms VimResized handler. All cases reach width 199. Measurements in `benchmarks/resize-coalescing-2026-09-26.json` measure RPC settling, not GUI display latency. The queue unit test holds a request in flight and verifies intermediate sizes are skipped and the final size arrives.

`cargo run --locked --example desktop_features` opens three disposable native windows. It verifies scrollback search across 200 lines, match counts, forward/backward selection and no matches. On macOS it verifies native Window menu titles/current checkmark and invokes Minimise, restores by selecting the listed window, and invokes Bring All to Front. It uses its own shells and closes its own application. A screenshot of the test window is attempted under `.cache/desktop-features.png`. Unicode replacement, deletion and composition are covered separately by input-state tests. These checks do not substitute for full manual keyboard/IME or Linux desktop validation.
