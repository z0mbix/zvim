...
import argparse, hashlib, json, pathlib, platform, shutil, subprocess, tarfile, zipfile
ROOT = pathlib.Path(__file__).resolve().parent.parent

def host():
    os = {'Darwin': 'macos', 'Linux': 'linux', 'Windows': 'windows'}[platform.system()]
    arch = 'arm64' if platform.machine().lower() in ('arm64', 'aarch64') else 'x86_64'
    return f'{os}-{arch}'

def bundle(target):
    names = {'macos-arm64': 'nvim-macos-arm64.tar.gz', 'macos-x86_64': 'nvim-macos-x86_64.tar.gz',
             'linux-x86_64': 'nvim-linux-x86_64.tar.gz', 'windows-x86_64': 'nvim-win64.zip'}
    name = names[target]
